...
name = "abiayi_2015-09-10-14-15-11_samsung-SM-T530_mdw_elicit_Dico5_7"
...
m1 = "/home/leferrae/Desktop/These/mboshi/crops/lex100/m13"
m2 = "/home/leferrae/Desktop/These/mboshi/crops/lex100/m1"
sent = get_mfcc_dd(file)
words_feat = [get_mfcc_dd(m1), get_mfcc_dd(m2)]
words_length = [words_feat[0].shape[0], words_feat[1].shape[0]]
...
matrix = cdist (m_vec,vect_segs, metric="cosine")
for cols in matrix:
    print(np.min(cols))
...
abs4 = []
for i in w07:
    abs7.append(matrix[0][i])
for i in w04:
    abs4.append(matrix[0][i])

###extract alignment###
with open(a_file, mode="r", encoding="utf8") as grid:
    ali = grid.read().split("\n")
words = []
for i in ali:
    u=i.split()
...
plt.plot(x, abs4, label="window 0.4")
plt.plot(x, abs7, label="window 0.7")
print()

legend = plt.legend(loc="upper right", shadow=True)
legend.get_frame().set_facecolor("C9")
plt.title('Distances for mwana')
